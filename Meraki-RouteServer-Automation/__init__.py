...
#Azure related config and environment variables
...
class Azure():
    ...
    def get_microsoft_network_base_url(self, azure_mgmt_url, subscription_id, resource_group=None, provider="Microsoft.Network"):
        if resource_group:
            return "{0}/subscriptions/{1}/resourceGroups/{2}/providers/{3}".format(azure_mgmt_url, subscription_id, resource_group, provider)
    ...
    def get_bearer_token(self, resource_uri):
        ...
        return header_with_bearer_token

    # function to obtain the route server information, we would need the routeserver asn and IP to peer with the vMXs. 
    def get_route_server(self, azure_mgmt_url, subscription_id, resource_group, route_server_name, header_with_bearer_token):
        endpoint_url = self.get_microsoft_network_base_url(azure_mgmt_url,subscription_id, resource_group) \
                                                     + f"/virtualHubs/{route_server_name}?api-version=2020-07-01"
    ...
    def get_route_server_bgp_connections(self, azure_mgmt_url, subscription_id, resource_group, route_server_name, header_with_bearer_token):
        endpoint_url = self.get_microsoft_network_base_url(azure_mgmt_url,
    ...
    def update_route_server_bgp_connections(self, azure_mgmt_url, subscription_id, resource_group, route_server_name, connection_name, \
...
class MerakiConfig(Azure):
    ...
    def get_tagged_networks(self):
        
        # executing API call to obtain all Meraki networks in the organization
        organization_networks_response = self.meraki_dashboard_sdk_auth.organizations.getOrganizationNetworks(
    ...
    def get_tagged_networks_bgp_data(self, network_id):
        header_with_bearer_token = super(MerakiConfig, self).get_bearer_token(azure_mgmt_url)
        route_server_config = super(MerakiConfig, self).get_route_server(azure_mgmt_url, subscription_id, resource_group, route_server_name, header_with_bearer_token)
        ...
        network_bgp_config = self.meraki_dashboard_sdk_auth.appliance.getNetworkApplianceVpnBgp(
            network_id
        )
        ...
        if network_bgp_config['enabled'] == False:
            ...
            enable_bgp_response = self.meraki_dashboard_sdk_auth.appliance.updateNetworkApplianceVpnBgp(
                network_id, enabled, 
                ibgpHoldTimer=180
            )
            update_bgp_config = self.meraki_dashboard_sdk_auth.appliance.updateNetworkApplianceVpnBgp(
                        network_id, enabled, 
                        ibgpHoldTimer=180,
        ...
        get_network_meraki_vpn_config = self.meraki_dashboard_sdk_auth.appliance.getNetworkApplianceVpnSiteToSiteVpn(
            network_id
        )
        ...
        if len(local_subnets_to_add) > 100:
            ...
            update_local_networks_response = self.meraki_dashboard_sdk_auth.appliance.updateNetworkApplianceVpnSiteToSiteVpn(
                network_id, mode, 
    ...
    def get_org_meraki_device_status(self):

        # using SDK to fetch the device status for every Meraki box in org
        device_status_response = self.meraki_dashboard_sdk_auth.organizations.getOrganizationDevicesStatuses(
            self.org_id, total_pages='all'
        )
...
def main(MerakiTimer: func.TimerRequest) -> None:
    ...
    azure_obj = Azure()
    meraki_obj = MerakiConfig()

    header_with_bearer_token = azure_obj.get_bearer_token(azure_mgmt_url)
    meraki_device_status = meraki_obj.get_org_meraki_device_status()

    # creating variable that is a list of all meraki networks inside the org
    org_networks = meraki_obj.get_tagged_networks()
    ...
    for networks in network_ids:
        ...
        network_bgp_info = meraki_obj.get_tagged_networks_bgp_data(networks[1])
    ...
    logging.info("List of Meraki vMX BGP Configs {0}".format(list_of_meraki_vmx_bgp_config))


    azure_route_server_bgp_connection_info = azure_obj.get_route_server_bgp_connections(azure_mgmt_url, \
                                             subscription_id, resource_group, route_server_name, header_with_bearer_token)
    ...
    azure_route_server_local_bgp_config = azure_obj.get_route_server(azure_mgmt_url, subscription_id, resource_group, route_server_name, header_with_bearer_token)
    ...
    for meraki_peers in list_of_meraki_vmx_bgp_config:
        ...
        for azure_peers in azure_route_server_bgp_connection_info['value']: 
            ...
            logging.info("List of Meraki Peers in Azure meraki_peers")
            ...
            # might have to un indent this one more time could be causing double puts to azure
                ...
                azure_obj.update_route_server_bgp_connections(azure_mgmt_url, subscription_id, resource_group, route_server_name, meraki_peers['network_name'], meraki_peers['uplink_ip'], meraki_peers['bgp_asn'], AZURE_TOKEN)
